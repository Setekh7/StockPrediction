...
model = Sequential()
model.add(Input(shape=(X_train.shape[1], 5)))  # Specify input shape correctly
...
model.fit(X_train, y_train, epochs=150, batch_size=32, validation_split=0.2, callbacks=[EarlyStopping(monitor='val_loss', patience=10)])
...
new_predictions_scaled = scaler.inverse_transform(np.column_stack((new_predictions, np.zeros((len(new_predictions), 4)))))[:, 0]
...
plt.plot(predicted_dates, new_predictions_scaled, color='green', label=f'Predicted Future {stock_symbol} Stock Price')
